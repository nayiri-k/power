...
main(int argc, char *argv[])
{
    ...
    char output_path[50]; strcpy(output_path,"./bin/");
    strcat(output_path, output_file);
    
    FILE *fptr;
    fptr = fopen(output_path, "wb");
    if (fptr == NULL) {
        fprintf(stderr, "ERROR: File does not exist, %s", output_path);
        return 0;
    }

    char *output_file2 = strcat(strtok(basename(argv[1]), "."),".txt");
    char output_path2[50]; strcpy(output_path2,"./out/idcodes-");
    strcat(output_path2, output_file2);
    FILE *fheader;
    fheader = fopen(output_path2, "r");
    if (fheader == NULL) {
        fprintf(stderr, "ERROR: File does not exist, %s", output_path2);
        return 0;
    } 
    // fprintf(stderr, "Reading idcodes from: %s\n", output_path2);
    // unsigned int idcode;
    // while (fscanf(fheader, "%u ", &idcode) == 1) {
    //     fprintf(stderr, "%u\n", idcode);
    // }   
    ...
    fwrite(&N_ROWS, sizeof(uint_T), 1, fptr);
    fwrite(&M_COLS, sizeof(uint_T), 1, fptr);
    ...
    while (fscanf(fheader, "%u ", &idcode) == 1) {
        ...
        fwrite(toggle_buff, sizeof(BUFFER_TYPE), M_COLS, fptr);
        // if (!all_zeros) {
        //     // write data to buffer
        //     fwrite(toggle_buff, sizeof(BUFFER_TYPE), M_COLS, fptr);
        // }        
