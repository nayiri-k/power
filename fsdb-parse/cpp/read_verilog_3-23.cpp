...
uint_T* buffer = malloc(10000*sizeof(float))
...
main(int argc, char *argv[])
{
        ...
        *prev_vc_ptr = *vc_ptr;
        ...
        do {
            ...
            if (time.L >= time_window) {
                ...
                if (time_window > max_time) max_time = time_window;
                
                #ifdef PRINT_CHANGE
                fprintf(stderr, "%u %u %f\n", 
                    time_window, num_toggles, (float) num_toggles/2);
                #endif

                #ifdef PRINT_ARRAY
                fprintf(stderr, "%f ",
                    (float) num_toggles/2/N_CYCLES);
                #endif
    ...
    // uint_T idx;
    // fprintf(stderr, "\n");
    // for (idx = 0; idx < max_time; idx+=CLOCK_PERIOD) {
