...
// [read_mix.cpp]
//
//  Copyright 2007-2009 SPRINGSOFT. All Rights Reserved.
//
...
// ****************************************************************************/
//
// Program Name	: read_mix.cpp
//
// Purpose	: Demonstrate how to call fsdb reader APIs to access 
//		  the value changes of verilog/vhdl mixed type fsdb.
//
...
static void
__PrintTimeValChngVerilog(ffrVCTrvsHdl vc_trvs_hdl,
                   fsdbTag64 *time, byte_T *vc_ptr);

static void
__PrintTimeValChngVHDL(ffrVCTrvsHdl vc_trvs_hdl,
                   fsdbTag64 *time, byte_T *vc_ptr);

static bool_T
__IsVerilogSignal(fsdbVarType var_type);

int 
main(int argc, char *argv[])
{
    if (2 != argc) {
	fprintf(stderr, "usage: read_mix mix_type_fsdb\n");
    ...
    if ( (FSDB_FT_VERILOG_VHDL != fsdb_info.file_type) &&
         (FSDB_FT_VERILOG      != fsdb_info.file_type) &&
         (FSDB_FT_VHDL         != fsdb_info.file_type) ) {
  	fprintf(stderr, "file type is not verilog/vhdl.\n");
	return FSDB_RC_FAILURE;
    }
    ...
    if ((FSDB_FT_VERILOG_VHDL != fsdb_obj->ffrGetFileType()) &&
        (FSDB_FT_VERILOG      != fsdb_obj->ffrGetFileType()) &&
        (FSDB_FT_VHDL         != fsdb_obj->ffrGetFileType())) {
	fprintf(stderr, 
		"%s is not verilog/vhdl type fsdb, just return.\n", argv[1]);
	fsdb_obj->ffrClose();
...
__IsVerilogSignal(fsdbVarType var_type)
{
    if (var_type >= FSDB_VT_VCD_EVENT &&
        var_type <= FSDB_VT_VCD_REG2)
        return TRUE; 
    else 
        return FALSE; 
}
...
{ 
    ...
    if (TRUE == __IsVerilogSignal(vc_trvs_hdl->ffrGetVarType())) {
        __PrintTimeValChngVerilog(vc_trvs_hdl, time, vc_ptr);
    }
    else {
        __PrintTimeValChngVHDL(vc_trvs_hdl, time, vc_ptr); 
    }
    return;
}
...
static bool_T 
__MyTreeCB(fsdbTreeCBType cb_type, 
           void *client_data, void *tree_cb_data)
{
    switch (cb_type) {
...
__DumpVar(fsdbTreeCBDataVar *var)
{
	...
	type = (str_T) "vhdl_signal";
	break;

    case FSDB_VT_VHDL_VARIABLE:
	type = (str_T) "vhdl_variable";
	break;

    case FSDB_VT_VHDL_CONSTANT:
	type = (str_T) "vhdl_constant";
	break;

    case FSDB_VT_VHDL_FILE:
	type = (str_T) "vhdl_file";
	break;
